...
public interface BasePacketRepository<E extends BasePacketEntity<?>, T> extends BaseRepository<E, T> {
  ...
  /**
	...
	@Query("SELECT dmve FROM DemoUpdateManualVerificationEntity dmve where dmve.id.regId=:regId and  dmve.statusCode=:statusCode")
	public List<E> getAllDemoAssignedRecord(@Param("regId") String regId, @Param("statusCode") String statusCode);
	...
	@Query("SELECT dmve FROM DemoUpdateManualVerificationEntity dmve where dmve.id.regId =:regId")
	public List<DemoUpdateManualVerificationEntity> getDemoVerificationEntity(@Param("regId") String regId);
	 ...
	 */
	@Query("SELECT bioRef FROM RegBioRefEntity bioRef WHERE bioRef.id.regId in :regIds")
	public List<RegBioRefEntity> getBioRefIdsByRegIds(@Param("regIds") List<String> regIds);
	...
	@Query(value ="SELECT m.reg_id FROM demo_update_manual_verification m WHERE m.request_id =:requestId",nativeQuery=true)
	public List<String> getDemoRegistrationIdbyRequestId(@Param("requestId") String requestId);
}
