...
public class OSIValidator {
	...
	public static final String INDIVIDUAL_TYPE_VID = "VID";
		...
		if (registrationStatusDto.getRegistrationType().equalsIgnoreCase(SyncTypeDto.NEW.name())
			...
			if (age < ageThreshold) {
				...
				if (isValidIntroducer(introducerVID)) {
					registrationStatusDto.setLatestTransactionStatusCode(registrationExceptionMapperUtil
							.getStatusCode(RegistrationExceptionTypeCode.PARENT_VID_NOT_IN_PACKET));
					registrationStatusDto.setStatusCode(RegistrationStatusCode.REJECTED.toString());
					registrationStatusDto.setStatusComment(StatusUtil.VID_NOT_FOUND.getMessage());
					registrationStatusDto.setSubStatusCode(StatusUtil.VID_NOT_FOUND.getCode());
					regProcLogger.debug(LoggerFileConstant.SESSIONID.toString(),
							LoggerFileConstant.REGISTRATIONID.toString(), registrationId,
							StatusMessage.PARENT_VID_NOT_IN_PACKET);
					return false;
				}
...
//
//				}
				if (introducerVID != null && !introducerVID.isEmpty()) {
					return validateIntroducerBiometric(registrationId, registrationStatusDto, introducerVID);
				} else {
					return false;
				}
			}
	...
	private boolean validateIntroducerBiometric(String registrationId,
			InternalRegistrationStatusDto registrationStatusDto,
			String introducerVID) throws IOException, ApisResourceAccessException,
		...
		if (biometricRecord != null && biometricRecord.getSegments() != null) {
			return validateUserBiometric(registrationId, introducerVID, biometricRecord.getSegments(),
					INDIVIDUAL_TYPE_VID, registrationStatusDto);
		} else {
			registrationStatusDto.setLatestTransactionStatusCode(registrationExceptionMapperUtil
	...
	private boolean isValidIntroducer(String introducerVID){
		return (introducerVID == null);
	}
