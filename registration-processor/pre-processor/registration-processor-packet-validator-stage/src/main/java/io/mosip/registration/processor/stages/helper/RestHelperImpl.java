...
public class RestHelperImpl implements RestHelper {
	...
	public boolean reactivateApplicant(String url, ActivateUinDto uin){
		...
		try {
			mosipLogger.info(LoggerFileConstant.SESSIONID.toString(), LoggerFileConstant.REGISTRATIONID.toString(), "",
					"RestHelperImpl::re activate url : "+ url);
			return (boolean) restApiClient.postApi(url, MediaType.APPLICATION_JSON,uin, boolean.class);
		}catch (Exception e){
			e.printStackTrace();
		}
	...
	public boolean isApplicantUinDeactivated(String url,String id){
		...
		mosipLogger.info(LoggerFileConstant.SESSIONID.toString(), LoggerFileConstant.REGISTRATIONID.toString(), "",
				"RestHelperImpl::uin status url : "+ url);
		if (url != null) {
		...
		try {
			...
			mosipLogger.info(LoggerFileConstant.SESSIONID.toString(), LoggerFileConstant.REGISTRATIONID.toString(), "",
					"RestHelperImpl::uin status  url builder : "+ uriComponents.toUri());
			return (boolean) restApiClient.getApi(uriComponents.toUri(), boolean.class);
		}catch (Exception e){
			e.printStackTrace();
		}
