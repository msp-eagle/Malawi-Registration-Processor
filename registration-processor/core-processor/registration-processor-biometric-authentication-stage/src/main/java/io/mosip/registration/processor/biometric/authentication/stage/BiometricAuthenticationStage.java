...
import io.mosip.registration.processor.biometric.authentication.exception.InvalidMessageException;
import io.mosip.registration.processor.biometric.authentication.response.dto.ManualAdjudicationResponseDTO;
import io.mosip.registration.processor.biometric.authentication.service.DemoUpdateService;
...
public class BiometricAuthenticationStage extends MosipVerticleAPIManager {
	...
	@Autowired
	private DemoUpdateService demoUpdateService;
	...
	/** The address. */
	@Value("${registration.processor.demo.queue.update.manualverification.response:demoupdatemv-to-mosip}")
	...
	public void consumerListener(Message message) {
		try {
			...
			regProcLogger.error("","","", io.mosip.kernel.core.exception.ExceptionUtils.getStackTrace(e));
		}
	...
	public MessageDTO process(MessageDTO object) {
		...
		try {
			...
			if (isTransactionSuccessful && demoOnlyUpdate) {
				demoUpdateService.pushRequestToQueue(object.getRid(),queue);
				object.setIsValid(Boolean.FALSE);
				object.setInternalError(Boolean.FALSE);
				registrationStatusDto.setStatusCode(RegistrationStatusCode.PROCESSING.name());
				registrationStatusDto
						.setLatestTransactionStatusCode(RegistrationTransactionStatusCode.MANUAL_QUEUE.toString());
	...
	private boolean idaAuthenticate(List<BIR> segments, String uin, InternalRegistrationStatusDto registrationStatusDto)
		...
		AuthResponseDTO authResponseDTO = authUtil.authByIdAuthentication(uin,
				uin.length() == uinLength ? BiometricAuthenticationConstants.INDIVIDUAL_TYPE_UIN : BiometricAuthenticationConstants.INDIVIDUAL_TYPE_VID, segments);
