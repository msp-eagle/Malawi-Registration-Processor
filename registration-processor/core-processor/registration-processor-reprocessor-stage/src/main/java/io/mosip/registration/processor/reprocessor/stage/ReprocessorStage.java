...
public class ReprocessorStage extends MosipVerticleAPIManager {
	...
	public void testConfig(){
		for(String s: reprocessExcludeStageNames){
			regProcLogger.info(LoggerFileConstant.SESSIONID.toString(), "test config",
					"", "exclude stage names : "+s);
		}

		regProcLogger.info(LoggerFileConstant.SESSIONID.toString(), "test config",
				"", "exclude stage names  list size : "+reprocessExcludeStageNames.size());
	}
			...
			if (!CollectionUtils.isEmpty(dtolist)) {
				...
				for (InternalRegistrationStatusDto dto : dtolist) {
					...
					if (reprocessCount.equals(dto.getReProcessRetryCount())) {
						...
						if(dto.getLatestTransactionTypeCode().equalsIgnoreCase("MANUAL_VERIFICATION") && reprocessExcludeStageNames.contains("ManualVerificationStage")) {
							regProcLogger.info("======================>" + "skipping manual verification rids = " + registrationId, "", "", "");
							continue;
						}
